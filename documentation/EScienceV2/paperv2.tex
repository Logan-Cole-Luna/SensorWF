...
\newcommand\Logan[1]{\leavevmode{\footnotesize\textbf{\hl{[Logan: #1]}}}}
\newcommand\Alex[1]{\leavevmode{\footnotesize\textbf{\hl{[Alex: #1]}}}}
\newcommand\Joe[1]{\leavevmode{\footnotesize\textbf{\hl{[Joe: #1]}}}}
\newcommand\Kellan[1]{\leavevmode{\footnotesize\textbf{\hl{[Kellan: #1]}}}}
...
\author{
  ...
  \IEEEauthorblockA{
    ...
    \{lambethl, rigoj1, shewk1\}@my.erau.edu,
...
Sensor-driven scientific disciplines encounter substantial computational challenges that have not been systematically addressed from an architectural perspective. Raw measurements must undergo ingestion, artifact correction, and transformation into analysis-ready feature representations. Each processing stage incorporates domain-specific assumptions about sampling rates, quality thresholds, and channel semantics, yet these assumptions are seldom explicitly documented or made machine-readable. Subsequent analyses further introduce domain-specific logic that builds upon this foundational structure.

Despite these structural similarities, pipelines in different domains are typically developed independently as monolithic, domain-specific scripts. This fragmentation hinders reuse, undermines the reliability of cross-domain comparisons, and makes post-publication auditing nearly impossible. The eScience and open-science communities have advocated for treating pipelines as first-class scientific objects with traceable, shareable, and reusable provenance \cite{wilkinson2016fair,garijo2022fair}.

This paper presents \textbf{SensorWF} (\emph{Generalizable FAIR Sensor Analytics Workflow}), a framework developed for multi-domain scientific sensor time-series analysis. The core architecture comprises five modules: ingestion (M1), quality assessment (M2), feature engineering (M3), semantic annotation (M4), and provenance export (M5). Domain-specific analysis tasks are incorporated as \emph{use-case extensions}, with synthetic fault injection (E1) and automated anomaly detection (E2) provided as demonstrations. Figure~\ref{fig:architecture} illustrates the complete architecture.

\subsection{Key Contributions}

The primary contributions of this work are as follows:

\Logan{Do we want this as an enumerated list or instead in paragraph form? I've seen other papers do either}
...
Sensor-oriented vocabularies such as SSN and SOSA address this issue by providing standardized concepts for describing sensors, observations, platforms, and related entities \cite{haller2019ssn,compton2012ssn}. These vocabularies allow sensor datasets to be represented using a shared conceptual structure while still permitting domain-specific extensions through ontologies. In this way, a workflow can preserve the specialized meaning of each domain, such as spacecraft subsystems or biomedical signal classes, while maintaining interoperability with broader semantic web standards.
...
\Alex{To Joe: Did you write these subsections? if not, please check if something is missing from your experience working with the cubesat}
...
\Alex{Kellan \& Joe: I feel we can conclude something from this paragraph, something like the location of the telemetry files should be documented using provenance (check the scientific provenance subsection). See if you have some ideas, those could be in the future work section.}
%% =========================================================
...
\Alex{To Logan: Have some json examples}
...
\textbf{Interoperable} & PROV-O + ProvONE; SSN/SOSA-aligned OWL; standardized DataFrame schema; typed module contracts \\
...
\textbf{Assumption-aware ML workflows.} Future work will extend SensorWF by representing additional workflow assumptions as machine-readable metadata, including preprocessing choices,
feature-construction parameters, detector configurations, and evaluation
settings. This would allow researchers to better inspect, compare, and
reuse ML-enabled sensor workflows across domains.
...
Special thanks to Dr. Jerry Sellers for his help in obtaining the satellite dataset. Special thanks to the College of Engineering at Embry-Riddle Aeronautical University for providing access to the CubeSat.
