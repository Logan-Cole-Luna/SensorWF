...
_ap.add_argument("--rerun",    action="store_true",
                 help="Ignore cached outputs and reprocess every experiment family")
...
for exp_name, analysis_file, debug_file in experiments:
    ...
    # ── Cache check ───────────────────────────────────────────────────────────
    _cdh_cached  = os.path.join(output_dir, "cdh_clean.csv")
    _qual_cached = os.path.join(output_dir, "quality_report.json")
    if not _ARGS.rerun and os.path.isfile(_cdh_cached) and os.path.isfile(_qual_cached):
        print(f"\n  [CACHE] {exp_name} — skipping (pass --rerun to force)")
        continue
...
print(f"  python use_case.py --domain satellite")
